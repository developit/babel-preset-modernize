/**
 * Remove polyfill implementations for known available APIs.
 * Ponyfill usage is also converted to native API usage where possible.
 *
 * @example
 *  // input:
 * 	var _assign = Object.assign || function() { ..etc.. };
 *  _assign(window, { name: 'hi' });
 *  // output:
 *  Object.assign(window, { name: 'hi' });
 */
...
export default function({ types: t, template }) {
	...
	function attemptInlineEvaluation(path, replacement) {
	  ...
	  */
		...
		for (let index = params.length; index--; ) {
			...
			binding.referencePaths.forEach(p => {
				//binding.dereference();
				p.replaceWith(t.clone(argValues[index]));
			});
	...
	*/
	...
	function isSingularFunctionBody(path) {
		if (t.isBlockStatement(path)) return isSingularFunctionBody(path.parentPath);
	...
	function handleFunctionConstructor(args) {
		args = args.slice();
		const body = args.pop();
		// @todo template.ast`{${body.node.value}}`
		const tpl = template('{' + body.node.value + '}');
	...
	return {
		...
		visitor: (visitor = {
			...
			MemberExpression(path) {
				...
				if (t.isIdentifier(obj) && /^(self|top|window|globalThis)$/.test(obj.node.name)) {
					const binding = obj.scope.getBinding(path.node.object.name);
					if (binding) {
						binding.dereference();

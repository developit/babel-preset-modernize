...
async function writeBeautified(filename, code) {
	await writeFile(filename, terser.minify(code, {
		mangle: false,
		compress: false,
		sourceMap: false,
...
describe('integration', () => {
	let summary = '', total = 0, sum = 0, sumEnv = 0;
	for (const f of readdirSync(FIXTURES)) {
		...
		it('should compile ' + f + ' to less code than preset-env', async () => {
			...
			try {
				// write to disk for easy inspection:
			...
			const sizeBefore = total += await compressedSize(before, { module: false });
			const size = sum += await compressedSize(out, { module: false });
			const sizeEnv = sumEnv += await compressedSize(outEnv, { module: false });
			summary += `\n${f}: ${bytes(size)}`;
			summary += chalk.dim(`\n  → ${sizeEnv-size}b smaller than preset-env's targets.esmodules (`) + chalk.green(`${(sizeEnv-size)/sizeEnv*100|0}% savings`) + chalk.dim(`)`);
			summary += chalk.dim(`\n  → ${sizeBefore-size}b smaller than preset-env's default (`) + chalk.green(`${(sizeBefore-size)/sizeBefore*100|0}% savings`) + chalk.dim(`)`);
			expect(size).toBeLessThan(sizeEnv);
		});
	}

	afterAll(() => {
		console.log(chalk.blueBright(`Compared to @babel/preset-env's targets.esmodules output:`) + summary);
	});
